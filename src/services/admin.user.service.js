...
const getUsers = async (params) => {
  ...
  if (search) {
    conditions.push(`(u.full_name ILIKE $${idx} OR u.email ILIKE $${idx} OR COALESCE(u.phone, '') ILIKE $${idx})`);
    values.push(`%${search}%`);
    idx++;
  }

  if (role) {
    if (!VALID_ROLES.includes(role)) throw new Error(`role phải là: ${VALID_ROLES.join(", ")}`);
  ...
  if (status) {
    if (!VALID_STATUSES.includes(status)) throw new Error(`status phải là: ${VALID_STATUSES.join(", ")}`);
...
const getUserById = async (userId) => {
  const result = await pool.query(Q.SELECT_USER_BY_ID, [userId]);
...
const updateUserStatus = async (userId, status, currentAdmin) => {
  ...
  try {
    ...
    const existingResult = await client.query(Q.SELECT_USER_FOR_STATUS_UPDATE, [userId]);
    if (existingResult.rows.length === 0) throw new Error("Không tìm thấy user");

    const existingUser = existingResult.rows[0];

    if (parseInt(existingUser.id, 10) === parseInt(currentAdmin.id, 10) && status !== "active") {
      throw new Error("Bạn không thể tự khóa hoặc vô hiệu hóa chính mình");
    }

    const shouldResetLoginState    = status === "active";
    const nextFailedLoginAttempts  = shouldResetLoginState ? 0 : existingUser.failed_login_attempts;
    const nextLockedUntil          = shouldResetLoginState ? null : existingUser.locked_until;

    const result = await client.query(Q.UPDATE_USER_STATUS, [
    ...
    return { message, user: sanitizeUser(result.rows[0]) };
  } catch (error) {
...
const updateUserRole = async (userId, role, currentAdmin) => {
  ...
  try {
    ...
    const existingResult = await client.query(Q.SELECT_USER_FOR_ROLE_UPDATE, [userId]);
    if (existingResult.rows.length === 0) throw new Error("Không tìm thấy user");

    const existingUser = existingResult.rows[0];

    if (parseInt(existingUser.id, 10) === parseInt(currentAdmin.id, 10) && role !== "admin") {
      throw new Error("Bạn không thể tự thay đổi role của chính mình");
    }

    const result = await client.query(Q.UPDATE_USER_ROLE, [role, userId]);

    await client.query("COMMIT");
    return { message: "Cập nhật role user thành công", user: sanitizeUser(result.rows[0]) };
  } catch (error) {
    await client.query("ROLLBACK");
    throw error;
  } finally {
