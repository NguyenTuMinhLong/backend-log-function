  ...
  rows.map((r) => ({
    ...
    seat: {
      ...
      price_breakdown: {
        ...
        infant_price: Math.round(parseFloat(r.base_price) * 0.10),
      },

      // Tổng tiền cho toàn bộ hành khách
      total_price: calcTotalPrice(parseFloat(r.base_price), adults, children, infants),
    },
  }));
...
const queryFlights = async ({
  ...
  min_price, max_price, airline_code, departure_city, arrival_city,
}) => {
  const a   = parseInt(adults)   || 1;
  const c   = parseInt(children) || 0;
  const i   = parseInt(infants)  || 0;
