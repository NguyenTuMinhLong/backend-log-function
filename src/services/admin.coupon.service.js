...
const ensureAirlineExists = async (airlineId) => {
  ...
  if (result.rows.length === 0) {
    throw new Error("airline_id không tồn tại");
  }
};
...
const validateCouponInput = async (data, isUpdate = false) => {
  ...
  if (!isUpdate || data.code !== undefined) {
    prepared.code = normalizeCode(data.code);
  }
  ...
  if (!isUpdate || data.value !== undefined) {
    ...
    if (value === null || value < 0) {
      throw new Error("value phải lớn hơn hoặc bằng 0");
    }
  ...
  if (!isUpdate || data.min_order !== undefined) {
    ...
    if (prepared.min_order !== null && prepared.min_order < 0) {
      throw new Error("min_order phải lớn hơn hoặc bằng 0");
    }
  }

  if (!isUpdate || data.max_discount !== undefined) {
    prepared.max_discount = parseOptionalNumber(data.max_discount, "max_discount");
    if (prepared.max_discount !== null && prepared.max_discount < 0) {
      throw new Error("max_discount phải lớn hơn hoặc bằng 0");
    }
  }

  if (!isUpdate || data.start_at !== undefined) {
    prepared.start_at = parseOptionalDate(data.start_at, "start_at");
  }

  if (!isUpdate || data.expiry_at !== undefined) {
    prepared.expiry_at = parseOptionalDate(data.expiry_at, "expiry_at");
  }

  if (!isUpdate || data.usage_limit !== undefined) {
    prepared.usage_limit = parseOptionalInteger(data.usage_limit, "usage_limit");
    if (prepared.usage_limit !== null && prepared.usage_limit < 0) {
      throw new Error("usage_limit phải lớn hơn hoặc bằng 0");
    }
  }

  if (!isUpdate || data.usage_limit_per_user !== undefined) {
    prepared.usage_limit_per_user = parseOptionalInteger(
      data.usage_limit_per_user,
      "usage_limit_per_user"
    );
    if (prepared.usage_limit_per_user !== null && prepared.usage_limit_per_user < 0) {
      throw new Error("usage_limit_per_user phải lớn hơn hoặc bằng 0");
    }
  }

  if (!isUpdate || data.welcome_only !== undefined) {
    prepared.welcome_only = parseOptionalBoolean(data.welcome_only, "welcome_only");
  }

  if (!isUpdate || data.is_active !== undefined) {
    prepared.is_active = parseOptionalBoolean(data.is_active, "is_active");
  }

  if (!isUpdate || data.name !== undefined) {
    prepared.name = data.name === undefined ? undefined : (data.name || null);
  }

  if (!isUpdate || data.description !== undefined) {
    prepared.description = data.description === undefined ? undefined : (data.description || null);
  }
...
const getCoupons = async (params) => {
  ...
  if (airline_id !== undefined && airline_id !== "") {
    ...
    values.push(parseOptionalInteger(airline_id, "airline_id"));
    idx++;
  }

  if (is_active !== undefined && is_active !== "") {
    conditions.push(`v.is_active = $${idx}`);
    values.push(parseOptionalBoolean(is_active, "is_active"));
    idx++;
  }

  if (welcome_only !== undefined && welcome_only !== "") {
    conditions.push(`v.welcome_only = $${idx}`);
    values.push(parseOptionalBoolean(welcome_only, "welcome_only"));
    idx++;
  }
...
const createCoupon = async (data) => {
  ...
  if (existing.rows.length > 0) {
    throw new Error(`Coupon với code "${prepared.code}" đã tồn tại`);
  }
  ...
  const result = await pool.query(
    `INSERT INTO vouchers (
     ...
     VALUES ($1, $2, $3, $4, COALESCE($5, 0), $6, $7, $8, $9, $10, COALESCE($11, FALSE), COALESCE($12, TRUE), $13, $14)
     RETURNING id`,
    [
      ...
      prepared.value,
      prepared.min_order,
      prepared.max_discount,
      prepared.start_at,
      prepared.expiry_at,
      prepared.usage_limit,
      prepared.usage_limit_per_user,
      prepared.welcome_only,
      prepared.is_active,
      prepared.name,
...
const updateCoupon = async (couponId, data) => {
  ...
  if (
    prepared.usage_limit !== undefined &&
    prepared.usage_limit !== null &&
...
const deleteCoupon = async (couponId) => {
  ...
  await pool.query("DELETE FROM vouchers WHERE id = $1", [couponId]);
