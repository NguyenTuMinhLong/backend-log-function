...
const validateBookingInput = (data) => {
  ...
  if (trip_type === "round_trip") {
    if (!return_flight_id)
      throw new Error("return_flight_id là bắt buộc cho khứ hồi");
    if (!return_seat_class)
      throw new Error("return_seat_class là bắt buộc cho khứ hồi");
  ...
  for (const p of passengers) {
    if (!p.full_name)
      throw new Error("full_name là bắt buộc cho mỗi hành khách");
    if (!p.passenger_type)
      throw new Error("passenger_type là bắt buộc cho mỗi hành khách");
 ...
 */
const checkAndGetSeatInfo = async (
  client,
  flightId,
  seatClass,
  seatsNeeded,
  ...
  if (result.rows.length === 0) {
    throw new Error(
      `Không tìm thấy hạng ghế ${seatClass} cho chuyến bay ID ${flightId}`,
    );
  }
  ...
  if (seat.status !== "scheduled") {
    throw new Error(
      `Chuyến bay ID ${flightId} không còn khả dụng (status: ${seat.status})`,
    );
  }
...
const createBooking = async (data, userId = null) => {
  ...
  try {
    ...
    // 3. Tính tổng tiền
    const outboundTotal = calcTotalPrice(
      parseFloat(outboundSeat.base_price),
      a,
      c,
      i,
    );
    ...
    const returnPassengers = passengers.filter(
      (p) => p.flight_type === "return",
    );
    ...
    for (const p of passengers) {
      ...
      const defaultBaggageKg = seatInfo
        ? parseInt(seatInfo.baggage_included_kg)
        : 23;
      ...
      if (!isInfant) {
        const flightId =
          flightType === "outbound" ? outbound_flight_id : return_flight_id;
        const seatClass =
          flightType === "outbound" ? outbound_seat_class : return_seat_class;
        const totalSeats =
          flightType === "outbound"
    ...
    return {
      ...
      baggage: {
        ...
        return_included_kg: returnSeat
          ? parseInt(returnSeat.baggage_included_kg)
          : null,
...
const getBookingDetail = async (bookingCode, userId = null) => {
  ...
  return {
    ...
    outbound_flight: {
      ...
      departure: {
        code: b.outbound_dep_code,
        city: b.outbound_dep_city,
        time: b.outbound_departure_time,
      },
      arrival: {
        code: b.outbound_arr_code,
        city: b.outbound_arr_city,
        time: b.outbound_arrival_time,
      },
    ...
    return_flight: b.return_flight_id
      ? {
          ...
          departure: {
            code: b.return_dep_code,
            city: b.return_dep_city,
            time: b.return_departure_time,
          },
          arrival: {
            code: b.return_arr_code,
            city: b.return_arr_city,
            time: b.return_arrival_time,
          },
...
const getMyBookings = async (userId, filter = "all") => {
  ...
  if (filter === "cancelled") {
    filterCondition = ` AND b.status = 'cancelled' `;
  }
...
const cancelBooking = async (userId, bookingCode) => {
  ...
  try {
    ...
    if (booking.status === "cancelled") {
      throw new Error("Booking này đã bị hủy rồi");
    }
    ...
    if (departureTime <= now) {
      throw new Error("Không thể hủy booking vì chuyến bay đã khởi hành");
    }
    ...
    if (seatAssignments.length > 0) {
      ...
      for (const key of Object.keys(groupedSeats)) {
        ...
        const releasedCount = groupedSeats[key];
        ...
        await client.query(
          ...
          WHERE flight_id = $2
            AND class = $3
          `,
          [releasedCount, flightId, seatClass],
        );
      }
    }
    ...
    return {
      ...
      policy: {
        hours_before_departure: Number(diffHours.toFixed(2)),
      ...
      payment: payment
        ? {
            ...
            new_status: paymentStatusAfterCancel,
...
const expireHeldBookings = async () => {
  ...
  try {
    ...
    for (const booking of expired.rows) {
      ...
      await client.query(
        `UPDATE flight_seat_assignments SET status='available', passenger_id=NULL, booking_id = NULL WHERE booking_id=$1`,
        [booking.id],
      );
    ...
    if (expired.rows.length > 0)
      console.log(
        `[Auto-expire] Đã hủy ${expired.rows.length} booking hết hạn`,
      );
  } catch (err) {
    await client.query("ROLLBACK");
